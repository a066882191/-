...
export default function LeaveRecordsPage() {
  ...
  const renderCalendar = useCallback(
    (isLightbox: boolean) => {
      ...
      const cellMinH = isLightbox ? 'min-h-[180px]' : 'min-h-[155px]';
      const textBase = isLightbox ? 'text-sm' : 'text-xs';
      const textSm = isLightbox ? 'text-xs' : 'text-[10px]';
      const textXs = isLightbox ? 'text-[10px]' : 'text-[9px]';
      const dayNumSize = isLightbox ? 'text-base' : 'text-sm';
      const gap = isLightbox ? 'gap-1' : 'gap-0.5';

      return (
        <div className="overflow-x-auto" style={{ fontFamily: '"DFKai-SB", "BiauKai", "標楷體", "KaiTi", serif' }}>
          <div className={isLightbox ? '' : 'min-w-[900px] text-center'}>
            {/* Weekday headers */}
            <div className="grid grid-cols-7 gap-0">
              {weekdays.map((w) => (
                <div
                  key={w}
                  className={`text-center ${textSm} font-bold text-stone-700 py-2 border border-black ${w === '日' || w === '六' ? 'bg-stone-200/70' : 'bg-stone-50'}`}
                >
                  {w}
                </div>
              ))}
            ...
            <div className="grid grid-cols-7 gap-0">
              {Array.from({ length: totalCells }).map((_, idx) => {
                ...
                const dayRequests = monthRequests
                  ...
                  .sort(sortByLeaveType)
                  .slice(0, isLightbox ? 12 : 8);
                const hasRequests = dayRequests.length > 0;

                return (
                  <div
                    key={idx}
                    className={`border border-black p-1.5 ${cellMinH} flex flex-col ${isWeekend ? 'bg-stone-50/80' : 'bg-white'}`}
                  >
                    {/* Day number header */}
                    <div
                      className={`${dayNumSize} font-bold mb-1 text-center border-b pb-0.5 flex items-center justify-center gap-1 ${hasRequests ? 'text-amber-700 border-amber-400' : 'text-stone-600 border-black'}`}
                    >
                      {isToday ? (
                      ...
                      <span className="text-stone-400 font-normal text-[10px]">週{weekdays[idx % 7]}</span>
                    </div>
                    ...
                    {hasRequests ? (
                      <div className="flex-1 space-y-0.5 overflow-hidden">
                        {dayRequests.map((req, ri) => {
                          ...
                          return (
                            <div
                              key={ri}
                              className={`relative flex items-center ${gap} ${textXs} leading-tight py-0.5 px-1 rounded border-b border-dashed border-stone-100 ${req.status === 'pending' && !dayReview ? 'bg-amber-50/60' : ''} ${isCancelled ? 'bg-stone-200/50' : ''}`}
                              title={`${req.employee_name} · ${typeInfo?.name || req.leave_type_name} · ${status.label}${dayReview ? ` (${dayReview === 'approved' ? '當日已核准' : '當日已駁回'})` : ''}${isCancelled ? ' (已取消申請)' : ''}`}
                            >
                              {isCancelled && (
                              ...
                              <span className={`font-semibold truncate ${isCancelled ? 'text-stone-400 line-through' : 'text-stone-700'}`}>{req.employee_name}</span>
                              <span className={`truncate flex-shrink-0 ${isCancelled ? 'text-stone-400 line-through' : 'text-stone-500'}`}>{typeInfo?.name || req.leave_type_name}</span>
                              <span className={`flex-shrink-0 ${isCancelled ? 'text-stone-400 line-through' : 'text-stone-400'}`}>{dt.full}</span>
                              {req.work_shift && (
                                <span className="text-stone-400 truncate flex-shrink-0 hidden sm:inline">{req.work_shift}</span>
                              )}
                              {isCancelled && (
                      ...
                      <div className="flex-1 flex items-center justify-center">
                        <span className={`${textXs} text-stone-300`}>-</span>
                      </div>
                    )}
                  </div>
                );
    ...
    <div className="min-h-screen bg-stone-50 pb-24">
      ...
      <div className="bg-white px-4 py-4 border-b border-stone-100 flex items-center gap-3">
        ...
        <div className="flex-1 flex items-center justify-between">
          <h1 className="text-lg font-bold text-stone-800">全體請假查詢</h1>
          ...
          {myPendingRequests.length === 0 ? (
            ...
            <div className="space-y-3">
              {myPendingRequests.map((req) => {
                ...
                return (
                  <div key={req.id} className={`bg-white rounded-xl p-4 border border-stone-100 relative ${req.status === 'cancelled' ? 'bg-stone-50/80' : ''}`}>
                    {req.status === 'cancelled' && (
                      <div className="absolute inset-y-0 left-0 right-0 flex items-center pointer-events-none z-10 px-16">
                        <div className="w-full h-[1.5px] bg-stone-400/70" />
                      </div>
                    )}
                    ...
                    <div className="relative flex items-center gap-2 mt-2.5">
                      ...
                      <div>
                        <p className={`text-sm font-medium ${req.status === 'cancelled' ? 'text-stone-400 line-through' : 'text-stone-800'}`}>{typeInfo?.name}</p>
                        <p className={`text-xs ${req.status === 'cancelled' ? 'text-stone-400 line-through' : 'text-stone-500'}`}>
